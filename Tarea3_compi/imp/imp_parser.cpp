...
Stm* Parser::parseStatement() {
  ...
  if (match(Token::ID)) {
    ...
    s = new ForStatement(lex,e,e2,tb); //FALTA IMPLEMENTAR
  } else {
...
Exp* Parser::parseBExp() {
  ...
  if (match(Token::AND) || match(Token::OR)){
    Token::Type op = previous->type;
    BinaryOp binop = (op==Token::AND)? AND : OR; // FALTA IMPLEMENTAR
    rhs = parseBExp();
    e = new BinaryExp(e, rhs, binop);
  }
  return e;
...
Exp* Parser::parseFactor() {
  ...
  /*
  if (match(Token::TRUE)) {
    return new BoolExp(true); // FALTA IMPLEMENTAR
  }
  if (match(Token::FALSE)) {
    return new BoolExp(false); // FALTA IMPLEMENTAR
  }
  */
  if (match(Token::LPAREN)) {
    Exp* e = parseExp();
